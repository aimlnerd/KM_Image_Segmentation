...
train_path = "./input/train/level-1/"
test_path = "./input/dev/level-1/"
...
def load_train(img_rows, img_cols):
    ...
    files = glob.glob(train_path+"*[0-9].tif")
    for fl in files:
        flbase = os.path.basename(fl)
        img = get_im_cv2(fl, img_rows, img_cols)
        X_train.append(img)
        X_train_id.append(flbase[:-4])
        mask_path = train_path + flbase[:-4] + "_mask.tif"
...
def load_test(img_rows, img_cols):
    ...
    files = glob.glob(test_path+"*[0-9].tif")
    X_test = []
    X_test_id = []
    total = 0
    start_time = time.time()
...
def find_best_mask():
    files = glob.glob(os.path.join(train_path, "*_mask.tif"))
...
def read_and_normalize_train_data(img_rows, img_cols):
    ...
    train_target = utils.to_categorical(train_target, 2)
...
def create_model(img_rows, img_cols):
    model = Sequential()
    model.add(Conv2D(40, 3, 3, padding='same', kernel_initializer='he_normal',
                            input_shape=(1, img_rows, img_cols)))
    #model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Dropout(0.15))

    model.add(Conv2D(80, 3, 3, padding='same', kernel_initializer='he_normal'))
    #model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Dropout(0.15))
...
def run_cross_validation(nfolds=10):
    img_rows, img_cols = 390, 390,#32, 32
    batch_size = 32
    nb_epoch = 500 #default 50 given
    random_state = 51
